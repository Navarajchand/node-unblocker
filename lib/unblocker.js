...
function Unblocker(config) {
  ...
  function handleRequest(clientRequest, clientResponse, next) {
    // Automatically redirect to Spotify if no path is specified
    if (clientRequest.url === "/" || clientRequest.url === config.prefix) {
      return clientResponse.redirectTo("https://open.spotify.com");
    }
